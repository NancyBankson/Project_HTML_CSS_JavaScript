...
export async function fetchData() {
    try {
        const response = await fetch("https://restcountries.com/v3.1/all?status=true&fields=name,popultion,region,capital,flags,subregion,currencies,languages,borders");
        if (!response.ok) {
